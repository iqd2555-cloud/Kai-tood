...
export function CashFlowManualForm({ today, branches, categories, entries, branchNameById, categoryNameByCode, saveAction, deleteAction }: Props) {
  ...
  const handleDelete = async (row: CashFlowEntry) => {
    ...
    if (!row.id || !row.dbPath) {
      const message = "ลบไม่ได้: รายการนี้ไม่มีรหัสเอกสารหรือ path ของฐานข้อมูล";
      setDeleteError(message);
      alert(message);
      console.error("Missing id/dbPath:", row);
      return;
    }

    if (row.source === "sales") {
      const message = "รายการนี้ต้องลบจากข้อมูลต้นทาง (ยอดขายหน้าร้าน)";
      setDeleteError(message);
      alert(message);
      console.warn("Blocked Cash Flow delete for generated sales row:", row);
      return;
    }
    ...
    try {
      console.log("Deleting cash flow row:", row);
      console.log("Delete dbPath:", row.dbPath);
      setDeletingId(row.id);

      const formData = new FormData();
      formData.set("entry_id", row.id);
      formData.set("db_path", row.dbPath);
      ...
      setLocalEntries((prev) => prev.filter((item) => item.dbPath !== row.dbPath));
