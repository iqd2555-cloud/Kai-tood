  ...
  return (
    <header className="sticky top-0 z-20 border-b border-black/10 bg-[#111111] text-white shadow-lg">
      <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-3">
        <Link href={staffOrderInputEnabled ? "/counter-orders" : "/dashboard"} className="focus-ring rounded-xl">
        ...
        <nav className="flex items-center gap-2 text-sm font-bold">
          {!staffOrderInputEnabled && <Link className="focus-ring rounded-full bg-white/10 px-3 py-2" href="/daily">กรอกข้อมูล</Link>}
          ...
          {!isOwner && !staffOrderInputEnabled && <Link className="focus-ring rounded-full bg-white/10 px-3 py-2" href="/my-reports">รายงานของฉัน</Link>}
          <form action={signOut}>
