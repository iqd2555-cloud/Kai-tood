...
async function reviewContent(formData: FormData) {
  ...
  if (!kpi) return;
  await kpi.from("content_automation_queue").update({ owner_status: decision, selection_status: decision === "approved" ? "selected" : "rejected", caption_status: decision === "approved" ? "pending" : "not_started", owner_reviewed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", id);
