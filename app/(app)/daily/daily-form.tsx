...
export function DailyForm({ branches, defaultBranchId, reportDate, existingReport, canSelectBranch }: { branches: DailyFormBranch[]; defaultBranchId: string; reportDate: string; existingReport?: DailyReport | null; canSelectBranch: boolean }) {
  ...
  const [receivedOtherItems, setReceivedOtherItems] = useState<{ name: string; amount: number }[]>(
    Array.isArray(existingReport?.received_other_items) && existingReport?.received_other_items.length > 0
      ? existingReport.received_other_items
      : [{ name: "", amount: 0 }],
  );
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <section id="received-inventory" className="scroll-mt-24 rounded-[1.75rem] border border-black/10 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-2 rounded-2xl border-2 border-dashed border-black/20 p-3">
          <p className="text-sm font-black text-black/60">วัตถุดิบรับเข้าอื่นๆ (ระบุชื่อและจำนวนเอง)</p>
          {receivedOtherItems.map((entry, index) => (
            <div key={index} className="grid gap-2 sm:grid-cols-[1fr_180px_auto]">
              <input className="focus-ring min-h-12 rounded-xl border-2 border-black/10 px-3 font-bold" placeholder="เช่น กล่องบรรจุ" value={entry.name} onChange={(event) => setReceivedOtherItems((prev) => prev.map((item, i) => i === index ? { ...item, name: event.target.value } : item))} />
              <input className="focus-ring min-h-12 rounded-xl border-2 border-black/10 px-3 text-center font-bold" type="number" inputMode="decimal" min="0" step="0.01" placeholder="จำนวน" value={entry.amount || ""} onChange={(event) => setReceivedOtherItems((prev) => prev.map((item, i) => i === index ? { ...item, amount: Number(event.target.value || 0) } : item))} />
              <button type="button" className="focus-ring rounded-xl bg-black px-4 text-sm font-black text-white" onClick={() => setReceivedOtherItems((prev) => prev.filter((_, i) => i !== index || prev.length === 1))}>ลบ</button>
            </div>
          ))}
          <input type="hidden" name="received_other_items" value={JSON.stringify(receivedOtherItems.filter((item) => item.name.trim() && item.amount > 0))} />
          <button type="button" className="focus-ring rounded-xl bg-[#ffc400] px-4 py-2 text-sm font-black" onClick={() => setReceivedOtherItems((prev) => [...prev, { name: "", amount: 0 }])}>+ เพิ่มรายการรับเข้าอื่นๆ</button>
        </div>
      </section>
