...
import { FreshChickenSalePanel } from "./fresh-chicken-sale-panel";
...
export default async function MarinationPage({ searchParams }: Props) {
  ...
  const [{ data: partsData, error: partsError }, { data: movementsData, error: movementsError }, { data: resetData, error: resetError }] = await Promise.all([
    ...
    supabase.from("marination_stock_movements").select("id, movement_date, chicken_part_id, movement_type, quantity_kg, note, created_by, created_at, updated_at, is_voided, voided_at, voided_by, void_reason").eq("is_voided", false).lte("movement_date", selectedDate).order("movement_date", { ascending: true }).order("created_at", { ascending: true }).order("id", { ascending: true }).returns<MarinationStockMovement[]>(),
    supabase.from("marination_stock_resets").select("id, reset_date, branch_id, note, created_at, created_by, is_active").eq("is_active", true).lte("reset_date", selectedDate).order("reset_date", { ascending: false }).order("created_at", { ascending: false }).limit(1).returns<MarinationStockReset[]>(),
  ]);
  ...
  if (partsError || movementsError || resetError) {
    return (
      <section className="rounded-[2rem] bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 font-bold text-black/60">กรุณาตรวจสอบตาราง chicken_parts และ marination_stock_movements ก่อนใช้งาน</p>
  ...
  const stockResetDate = resetData?.[0]?.reset_date ?? null;

  return (
    <div className="space-y-5">
      <FreshChickenSalePanel parts={parts} movements={movements} selectedDate={selectedDate} stockResetDate={stockResetDate} />
      <MarinationConsole
        parts={parts}
