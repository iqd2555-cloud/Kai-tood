...
export function ImageQuotePreview({ queueId, imageUrl, quote, rendered = false }: Props) {
  ...
  async function renderAndSave() {
    ...
    try {
      ...
      if (!ctx) throw new Error("ไม่สามารถสร้างไฟล์รูปได้");

      ctx.drawImage(image, 0, 0, width, height);
      const bandHeight = Math.max(90, Math.round(height * 0.15));
      ...
      while (fontSize >= 24) {
        ...
        if (lines.length <= 3 && lines.length * lineHeight <= bandHeight * 0.74) break;
        fontSize -= 2;
      }
      ...
      let y = bandTop + (bandHeight - totalHeight) / 2 + lineHeight / 2;
      ...
      const blob = await new Promise<Blob>((resolve, reject) => {
        canvas.toBlob((value) => value ? resolve(value) : reject(new Error("สร้างไฟล์ JPEG ไม่สำเร็จ")), "image/jpeg", 0.92);
      });
      const body = new FormData();
      body.append("queue_id", queueId);
      body.append("file", new File([blob], "post-ready.jpg", { type: "image/jpeg" }));
  ...
  return <div className="space-y-2">
    <div className="relative overflow-hidden rounded-2xl bg-black">
      ...
      {!rendered && quote ? <div className="pointer-events-none absolute inset-x-0 bottom-0 flex min-h-[15%] items-center justify-center bg-black/70 px-5 py-3 text-center text-lg font-black leading-snug text-white drop-shadow">{quote}</div> : null}
    </div>
