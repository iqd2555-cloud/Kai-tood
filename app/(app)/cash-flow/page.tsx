...
export default async function CashFlowPage({ searchParams }: PageProps) {
  ...
  return <div className="space-y-5">
    <section className="rounded-[2rem] bg-[#111] p-5 text-white shadow-lg">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-black text-[#FFD43B]">Cash Flow Center</p><h1 className="text-3xl font-black">ศูนย์บริหารกระแสเงินสด</h1><p className="mt-2 text-sm font-bold text-white/70">ดูเงินจริง รอรับ รอจ่าย และคาดการณ์ 7/30 วัน แบบไม่ซับซ้อนเหมือนบัญชีภาษี</p></div><form action={async (formData: FormData) => { "use server"; const result = await syncSalesToCashFlow(formData); const message = encodeURIComponent(result.message); redirect(`/cash-flow?from=${formData.get("from") ?? from}&to=${formData.get("to") ?? to}&sync_ok=${result.ok ? "1" : "0"}&sync_message=${message}`); }} className="flex flex-col gap-2 sm:items-end"><input type="hidden" name="from" value={from}/><input type="hidden" name="to" value={to}/><button className="focus-ring rounded-full bg-[#FFD43B] px-5 py-3 font-black text-black">ซิงก์ยอดขายอัตโนมัติ</button><p className="text-xs font-bold text-white/60">ดึงยอดขายตามช่วงวันที่ที่เลือก แล้ว upsert ไม่ให้ซ้ำ</p></form></div>
    </section>
    ...
    <section className="rounded-[1.75rem] border border-black/10 bg-white p-5 shadow-sm"><h2 className="text-2xl font-black">บันทึกรายการเอง</h2><form action={async (formData: FormData) => { "use server"; const result = await saveCashFlowEntry(null, formData); const message = encodeURIComponent(result.message); redirect(`/cash-flow?from=${formData.get("transaction_date")}&to=${formData.get("transaction_date")}&cash_flow_ok=${result.ok ? "1" : "0"}&cash_flow_message=${message}`); }} className="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      <Field label="หมวดหมู่"><select className={inputClass()} name="category"><option value="">ไม่ระบุ</option>{categories?.map((c)=><option key={c.id} value={c.name}>{c.name}</option>)}</select></Field><Field label="ช่องทางเงิน"><input className={inputClass()} name="payment_method" placeholder="เช่น เงินสด / โอน / ธนาคาร"/></Field>
