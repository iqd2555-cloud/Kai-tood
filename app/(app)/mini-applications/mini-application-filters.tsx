...
import { thaiDistrictsByProvince, thaiProvinces } from "@/lib/thai-address";
...
type Props = { q: string; province: string; district: string; status: string; from: string; to: string };

export function MiniApplicationFilters(props: Props) {
  const [province, setProvince] = useState(props.province);
  const districts = province && province in thaiDistrictsByProvince ? thaiDistrictsByProvince[province as keyof typeof thaiDistrictsByProvince] : [];

  return <form className="grid gap-3 rounded-[1.5rem] bg-white p-4 shadow-sm sm:grid-cols-3">
    <input name="q" defaultValue={props.q} placeholder="ค้นหาชื่อ เบอร์โทร เลขอ้างอิง" className={inputClass} />
    <select name="province" value={province} onChange={(event) => setProvince(event.currentTarget.value)} className={inputClass}>
      <option value="">ทุกจังหวัด</option>{thaiProvinces.map((item) => <option key={item} value={item}>{item}</option>)}
    </select>
    <select key={province} name="district" defaultValue={province === props.province ? props.district : ""} disabled={!province} className={inputClass}>
    ...
    <button className="rounded-2xl bg-black px-5 py-3 font-black text-white sm:col-span-3">ค้นหา</button>
  </form>;
}
