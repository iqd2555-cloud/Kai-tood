-- Counter Order SQL Editor full repair migration.
-- Copy and paste this entire file into the Supabase SQL Editor when the
-- application still reports after NOTIFY pgrst reload:
--   กรุณารัน migration ล่าสุดของระบบนับออเดอร์ แล้วสั่ง reload schema
--
-- This script is intentionally idempotent. It creates every Counter Order
-- table, enum, seed row, RLS policy, grant, and RPC required by the app, then
-- verifies the required objects and reloads the PostgREST schema cache.
-- It is safe to run again if a previous attempt partially succeeded.

create extension if not exists "pgcrypto";

-- Counter Order depends on the existing app base schema. Fail early with a
-- readable message if the production database has not received the core app
-- migrations yet.
do $$
declare
  missing_dependencies text[];
begin
  select coalesce(array_agg(name), '{}') into missing_dependencies
  from (
    values
      ('table', 'public.branches'),
      ('table', 'public.profiles'),
      ('function', 'public.current_profile_role()'),
      ('function', 'public.current_profile_branch_id()')
  ) as required_dependency(kind, name)
  where (kind = 'table' and to_regclass(name) is null)
     or (kind = 'function' and to_regprocedure(name) is null);

  if array_length(missing_dependencies, 1) is not null then
    raise exception 'Counter Order migration requires base app objects first. Missing: %', array_to_string(missing_dependencies, ', ');
  end if;
end $$;
...
-- Drop and recreate RPCs so the SQL Editor script also repairs older databases
-- that already have a stale function with the same arguments but a different
-- return type. `create or replace function` alone cannot change return types.
drop function if exists public.reprint_order(uuid);
drop function if exists public.mark_order_printed(uuid);
drop function if exists public.cancel_latest_counter_order(uuid, text);
drop function if exists public.create_counter_order(uuid, numeric, integer);
drop function if exists public.can_use_counter_branch(uuid);
drop function if exists public.get_counter_price_items();
...
-- Verification output: every required table/function below must show `true`.
...
select 'table' as object_type, name as object_name, (to_regclass(name) is not null) as exists
from required_tables
union all
select 'function' as object_type, signature as object_name, (to_regprocedure(signature) is not null) as exists
